...
public class SquareArrayAnalyzer {
    ...
    public static Boolean compare(Integer[] inputArray, Integer[] squaredValues) {
        Integer[] inputSquared = new Integer[inputArray.length];
        for (int i = 0; i < inputArray.length; i++){
            inputSquared[i] = inputArray[i] * inputArray[i];

        }
        Arrays.sort(inputSquared);
        Arrays.sort(squaredValues);
        return Arrays.equals(inputSquared, squaredValues);
